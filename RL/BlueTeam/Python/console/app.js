...
function connectionStatus(){
 ...
 $('connection').replaceChildren(document.createElement('i'),document.createTextNode(label));
 $('connection').firstChild.style.background=mode==='live'&&!connected?'#ffb18a':'#6ce8c8';
 $('plan').disabled=!connected||busy;$('connect').textContent=connected?'Disconnect':'Connect to Unreal';
}
function controls(){
 $('play').textContent=playing?'Pause':mode==='live'?'Run episode':'Play replay';
 $('play').disabled=!view||busy||Boolean(mode==='live'&&view.ended);
 $('step').disabled=!view||playing||busy||Boolean(mode==='live'&&view.ended);
 $('restart').disabled=!view||mode==='live'||busy;
 $('timeline').disabled=!view||mode==='live';$('speed').disabled=mode==='live';
 $('connect').disabled=busy;$('plan').disabled=!connected||busy;
 $('recorded-mode').disabled=busy;$('live-mode').disabled=busy;
}
...
function render(){
 const frame=view?.frames[frameIndex];$('timeline').value=frameIndex;
 $('clock').textContent=timeText(frame?.time||0);
 $('progress').textContent=view?mode==='live'?`Step ${frame.completedSteps??0} · ${view.ended?'ended':'paused between requests'}`:`Frame ${frameIndex+1} / ${view.frames.length} · ${fmt(view.frames.at(-1).time)} s`:'No episode';
...
function events(frame){
 ...
 log.sort((a,b)=>b[0]-a[0]);$('event-count').textContent=log.length;$('events').replaceChildren();
 for(const [t,text] of log.slice(0,40)){const row=element('div','','event');row.append(element('time',`${fmt(t)} s`),element('span',text));$('events').append(row);}
...
function draw(frame){
 ...
 for(const t of frame.threats){const [x,y]=xy(t.position);if(t.detected||t.tracked)circle([x,y],10,null,t.tracked?'#6ce8c8':'#f4cf78');ctx.save();ctx.translate(x,y);ctx.rotate(Math.PI/4);ctx.fillStyle=t.breached?'#ff5757':t.tracked?'#6ce8c8':'#ff8b80';ctx.fillRect(-3.5,-3.5,7,7);ctx.restore();if(w>470&&frame.threats.length<=8)label(t.id,x+12,y+4,'#c2ccda');}
 for(const t of frame.tracks||[]){const p=xy(t.position);circle(p,9,null,t.confirmed?'#6ce8c8':'#f4cf78');if(w>470)label(t.id,p[0]+12,p[1]-9,'#f4cf78');}
}
...
function animate(now){
 if(playing&&view){
  ...
  }else if(!busy&&now-liveWall>=500){liveWall=now;liveAction('step');}
 }
