...
def test_http_session_replay_and_security_headers(http_server):
    ...
    assert {row['id'] for row in session['trainingAlgorithms']} == {'local_ppo', 'paired_bandit', 'reinforce', 'ppo', 'a2c'}
...
def test_compare_tab_offers_measured_eight_sensor_results_with_contract_caveat():
    ...
    assert 'comparison.layouts=session.comparisonLayouts||comparison.layouts' in script
