"""Archive the balanced-v3 validation experiment without promoting its candidate.

No network, training, simulation rollouts, source edits or default-policy changes
occur. Optional STOP probes are verified by repeating public-only actor inference.
...
import evaluate_balanced as selector
from evaluate_balanced import V1_WEIGHTS, V2_WEIGHTS, checkpoint_files, implementation_fingerprints, validate_seed_range
from publish_robust_rl import (sha256, strict_json, privacy_check, _json_object, _gzip_json,
                               _canonical_bytes, _PRIVATE_PATH, _SECRET, RUN_FILES)
from triad_rl.adaptive_evaluation import canonical_hash, LEGACY_PARAMETER_SHA256
...
SCHEMA = "triad.balanced_publication.v1"
...
def _source_files(additional: Sequence[str] = ()) -> dict[str, str]:
    ...
    sources["Tools/publish_balanced_rl.py"] = sha256(Path(__file__).read_bytes())
    sources["Tools/publish_robust_rl.py"] = sha256((REPO_ROOT / "Tools" / "publish_robust_rl.py").read_bytes())
...
def _validate_demo(data: bytes, weights: str, selected_files: dict[str, str], lineage: dict) -> None:
    from demo_balanced import implementation_fingerprints as demo_sources
    ...
    for replay in envelope["replays"]:
        ...
        if (replay.get("schema") != "triad.balanced_replay.v1" or replay.get("release") != "balanced-v3"
                or replay.get("stage") != "validation" or not replay.get("frames")
                or audit.get("policy_schema") != POLICY_SCHEMA
                or audit.get("deterministic_rule") != "STOP probability >= 0.5, else best legal deployment"
...
def _validate_stop_probe(data: bytes, expected: dict) -> dict:
    """Recompute each first action using the versioned actor, never private truth."""
    ...
    if (evidence.get("schema") != "triad.balanced_stop_probe.v1"
            or evidence.get("stage") != "validation"
            or evidence.get("policy_kind") != expected["policy_kind"]
            or evidence.get("weights_sha256") != expected["weights_sha256"]
            or evidence.get("checkpoint_files_sha256_before") != expected["files"]):
        raise ValueError("STOP probe differs from its declared versioned checkpoint")
    validate_seed_range(interval.get("start"), interval.get("count"), "validation")
    if interval["count"] > 1000 or len(evidence.get("records", [])) != interval["count"]:
        raise ValueError("STOP probe must contain a bounded complete set of at most 1000 records")
    actual = run_probe(expected["path"], policy_kind=expected["policy_kind"],
                       seed=interval["start"], episodes=interval["count"],
                       profile=evidence.get("profile"))
    if not _same_derived(evidence, actual):
        raise ValueError("STOP probe differs from recomputed public inputs, actual actor actions, summaries or hashes")
    return evidence


def _same_derived(left: Any, right: Any) -> bool:
    """Permit only last-bit floating drift; hashes/actions/counts stay exact.

    NumPy/BLAS implementations can differ in their final floating bits across
    supported platforms. This tolerance is for recomputation only; original
    artifact bytes and their cryptographic integrity are never changed.
    """
    ...
    if isinstance(left, dict):
        return left.keys() == right.keys() and all(_same_derived(left[key], right[key]) for key in left)
    if isinstance(left, list):
        return len(left) == len(right) and all(_same_derived(a, b) for a, b in zip(left, right))
    return left == right


def _validate_comparisons(report: dict, selected_name: str, first_name: str) -> None:
    """Recompute both inherited and selected descriptive paired intervals."""
    samples = set()
    for field, reference in (("paired_differences", first_name),
                             ("selected_candidate_paired_differences", selected_name)):
        comparisons = report.get(field)
        if not isinstance(comparisons, dict) or not comparisons:
            raise ValueError("Validation report lacks complete paired comparisons")
        for metrics in comparisons.values():
            if not isinstance(metrics, dict) or not metrics:
                raise ValueError("Invalid paired comparison metric set")
            for interval in metrics.values():
                count = interval.get("bootstrap_samples") if isinstance(interval, dict) else None
                if isinstance(count, bool) or not isinstance(count, int) or not 1 <= count <= 100000:
                    raise ValueError("Invalid bounded paired bootstrap count")
                samples.add(count)
        if len(samples) != 1:
            raise ValueError("Paired comparisons must use one consistent bootstrap count")
        if not _same_derived(selector._selected_comparisons(report, reference, next(iter(samples))), comparisons):
            raise ValueError("Paired comparison intervals differ from complete episode records")
...
def publish(*, runs: Mapping[int, str | Path], selection_dir: str | Path,
    ...
    Production balanced-v3 declares seeds 201/202/203 and 8000 episodes each.
    ...
    extra_sources = (["demo_balanced.py", "demo_robust.py", "demo_adaptive.py", "adaptive_demo.html"] if demo is not None else [])
    if stop_probes:
        extra_sources.extend(["probe_balanced.py", "probe_stop_exploration.py"])
    ...
    if (declared.get("schema") != "triad.balanced_experiment_protocol.v1"
    ...
    frozen = declared.get("frozen_reference", {})
    ...
    if (selection.get("schema") != "triad.balanced_model_selection.v1"
    ...
    for seed, run in sorted(paths.items()):
        ...
        evidence = selector._run_evidence(run / "best", policy, seed, declared)
    ...
    old_lineage, old_evidence = selector._reference_evidence()
    reference_paths = {"robust_v2": BLUE_ROOT / "Checkpoints" / "robust-v2-candidate",
                       "adaptive_v1": BLUE_ROOT / "Checkpoints" / "adaptive-v1",
                       "legacy_toy210_projected": BLUE_ROOT / "Checkpoints" / "toy-210"}
    reference_files = {}
    references = {}
    for name, path in reference_paths.items():
        for filename in ("checkpoint.json", "arrays.npz"):
            read(path / filename)
        reference_files[name] = checkpoint_files(path)
        if name != "legacy_toy210_projected":
            references[name] = AdaptivePolicy.load(path, feature_names=FEATURE_NAMES)
    if (selection.get("reference_evidence") != old_evidence
            or selection.get("reference_checkpoint_files_sha256") != reference_files
            or references["adaptive_v1"].weights_fingerprint() != V1_WEIGHTS
            or references["robust_v2"].weights_fingerprint() != V2_WEIGHTS):
        raise ValueError("Selection reference evidence/checkpoints differ from frozen v1/v2/legacy baselines")
    lineage = {**old_lineage,
               "reference_checkpoints": {name: policy.metadata for name, policy in references.items()},
               "candidate_checkpoints": {str(seed): info["metadata"] for seed, info in actual_candidates.items()},
    ...
    for profile in PROFILES:
        ...
        if (report.get("implementation_sha256") != implementation_fingerprints()
                ...
                or report.get("reference_evidence") != old_evidence
                or set(report.get("methods", {})) != {*(f"candidate_{seed}" for seed in paths),
                    *(f"initialized_{seed}" for seed in paths), "adaptive_v1", "robust_v2",
                    "greedy_public", "random_legal", "uniform_fixed_rf_radar",
                    "fixed_rf", "fixed_radar", "legacy_toy210_projected"}):
        ...
        for reference_name, fingerprint in (("adaptive_v1", V1_WEIGHTS), ("robust_v2", V2_WEIGHTS),
                                            ("legacy_toy210_projected", LEGACY_PARAMETER_SHA256)):
            for record in report["methods"][reference_name]["episodes"]:
                if any(record.get(key) != fingerprint for key in ("weights_sha256_before", "weights_sha256_after")):
                    raise ValueError("Validation reference weights differ from the frozen baseline")
        reports[profile] = report
        add(f"{RESULTS}/{filename}", data)
    selected, scores = selector.select_from_reports(reports, actual_candidates)
    ...
    for report in reports.values():
        _validate_comparisons(report, f"candidate_{selected['seed']}", f"candidate_{min(paths)}")
    expected_references = {profile: {name: method["summary"] for name, method in report["methods"].items()
                                     if not name.startswith("candidate_")} for profile, report in reports.items()}
    ...
    for role, destination in (("best", "balanced-v3-candidate"), ("initialized", "balanced-v3-candidate-initialized")):
        for filename in ("checkpoint.json", "arrays.npz"):
    ...
    probe_descriptions = []
    initialized_path = paths[selected_seed] / "initialized"
    initialized = BalancedPolicy.load(initialized_path, feature_names=FEATURE_NAMES)
    allowed_probes = [
        {"weights_sha256": V2_WEIGHTS, "files": reference_files["robust_v2"],
         "path": reference_paths["robust_v2"], "policy_kind": "adaptive", "role": "frozen_v2"},
        {"weights_sha256": initialized.weights_fingerprint(), "files": checkpoint_files(initialized_path),
         "path": initialized_path, "policy_kind": "balanced", "role": "balanced_initialized"},
        {"weights_sha256": weights, "files": actual_candidates[selected_seed]["checkpoint_files_sha256"],
         "path": paths[selected_seed] / "best", "policy_kind": "balanced", "role": "balanced_selected"}]
    if stop_probes and len(stop_probes) != 3:
        raise ValueError("Supply exactly three paired probes in frozen-v2, balanced-initialized, balanced-selected order")
    pairing_key, pairing_hashes = None, None
    for index, probe_path in enumerate(stop_probes):
        data = read(Path(probe_path).resolve())
        probe = _validate_stop_probe(data, allowed_probes[index])
        interval = probe["seed_provenance"]["validation_diagnostic"]
        key = (probe["profile"], interval["start"], interval["count"])
        fingerprints = [row["public_input_sha256"] for row in probe["records"]]
        if index and (key != pairing_key or fingerprints != pairing_hashes):
            raise ValueError("STOP probes must use identical paired public inputs")
        pairing_key, pairing_hashes = key, fingerprints
        name = f"{RESULTS}/stop-probe-{index + 1}.json"
        add(name, data)
        probe_descriptions.append({"path": name, "role": allowed_probes[index]["role"],
                                   "weights_sha256": probe["weights_sha256"], "profile": probe["profile"],
                                   "seed_range": interval, "independent_unseen_evidence": False})
    manifest = {"schema": SCHEMA, "release": "balanced-v3", "stage": "validation",
                ...
                "initialized_weights_sha256": initialized.weights_fingerprint(), "archived_run_seeds": sorted(paths),
                ...
                "recomputation_float_tolerance": {"relative": 1e-12, "absolute": 1e-12,
                                                   "exact": "hashes, schemas, actions, counts and artifact bytes"},
    ...
    if (_source_files(extra_sources) != sources_before
            or selector._reference_evidence() != (old_lineage, old_evidence)
            or any(path.read_bytes() != data for path, data in source_inputs.items())):
    ...
    for name, data in artifacts.items():
        ...
        if path in resolved_names:
            raise ValueError("Multiple publication artifact names resolve to the same destination")
        resolved_names[path] = name
